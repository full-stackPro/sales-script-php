...
(function($, d, w) {
        ...
        bubble = {
            ...
            updatePos: function(editor, elem) {
                var sel = w.getSelection(),
                    ...
                    var screen_width = $(window).width() -  bubbleWidth;
            ...
            buildMenu: function(editor, elem) {
                ...
                for (var cmd in options.modifiers) {
					...
					if(options.modifiers[cmd] == 'fontSize' || options.modifiers[cmd] == 'fontFamily'){
						var fonts = [{"Arial,Helvetica,sans-serif":"Arial"}, {"Georgia,serif":"Georgia"},{"Impact,Charcoal,sans-serif":"Impact"},{"Tahoma,Geneva,sans-serif":"Tahoma"},{"Times New Roman,Times,serif,-webkit-standard":"Times New Roman"},{"Verdana,Geneva,sans-serif":"Verdana"}];
							if(options.modifiers[cmd] == 'fontFamily'){
								var fontFamily_options = ''; 
								$.each(fonts,function(key,val){
									
									$.each(val,function(k,v){
									fontFamily_options += '<option style="font-family:'+k+'" value="'+k+'">'+v+'</option>';
									});
								});
								var btn = utils.html.addTag(li, 'select', false, false);
								btn.append(fontFamily_options);
							}else if(options.modifiers[cmd] == 'fontSize'){ 
								var font_sizes = ["10","12","13","14","15","16","17","18","19","20","22","24","26","28","30","34","36","38","40","44","48","52","60","72"];
								var fontSize_options = ''; 
								for (var i in font_sizes) {
									fontSize_options += '<option value="'+font_sizes[i]+'">'+font_sizes[i]+'</option>';
								}
								var btn = utils.html.addTag(li, 'select', false, false);
								btn.append(fontSize_options);
							}
					}else{
				...
				elem.find('select').on('change',function(e) {
                    ...
                    events.commands[cmd].call(editor, e,$(this).val());
                });
        ...
        events = {
            commands: {
                bold: function(e) {
					...
					/*console.log(utils.selection.getSelection());
                    if(!$(window.getSelection().focusNode.parentNode).is('strong')){
                       var txt = utils.selection.getText();
    					console.log(utils.selection.getSelection());
                        //d.execCommand('fontSize', false,val);
    					if(txt != ''){
    						d.execCommand('insertHTML', false, '<strong>'+txt+'</strong>');
    					}
                    }else{
                        var txt = utils.selection.getText();
    					console.log(utils.selection.getSelection());
                        //d.execCommand('fontSize', false,val);
    					if(txt != ''){
    						d.execCommand('insertHTML', false, '<span>'+txt+'</span>');
    					}
                    }
					*/
				...
				fontSize: function(e,val) {
					...
					//console.log(txt);
					$(".editor").html(txt.replace(sel, '<span style="font-size:'+val+'px">'+sel+'</span>'));
					
					/*var txt = utils.selection.getText();
					console.log(utils.selection.getSelection());
                    //d.execCommand('fontSize', f alse,val);
					if(!$(window.getSelection().focusNode.parentNode).is('span')){
						if(txt != ''){
							d.execCommand('insertHTML', false, '<span style="font-size:'+val+'px">'+txt+'</span>');
						}
					}else{
						var myspan = $(window.getSelection().focusNode.parentNode);
						var style  = myspan.attr("style");
						style  += ';font-size:'+val+'px';
						
						myspan.attr("style",style);
						console.log(myspan[0].outerHTML);
						d.execCommand('insertHTML', false,myspan[0].outerHTML);
					}*/
                    bubble.update.call(this);
                    events.change.call(this);
                },
				fontFamily: function(e,val) {
                    e.preventDefault();
                    d.execCommand('fontName', false,val);
					/*
					var txt = utils.selection.getText();
					
					if(!$(window.getSelection().focusNode.parentNode).is('span')){
						if(txt != ''){
							d.execCommand('insertHTML', false, '<span style="font-family:'+val+'">'+txt+'</span>');
						}
					}else{
						var myspan = $(window.getSelection().focusNode.parentNode);
						var style  = myspan.attr("style");
    ...
    $.fn.notebook.defaults = {
        ...
        mode: 'inline', 
        modifiers: ['bold', 'italic', 'underline', 'fontSize','fontFamily']
    };
