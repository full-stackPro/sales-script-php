$(function() {
	...
	$('[data-popup-open]').on('click', function(e) {
		...
		clone_ele.find('.buttons').attr("id", "newelement");
...
$(document).ready(function() {
	...
	$(".inline-buttons").on('click','.buttons',function () {
	  var p= $("#"+this.value);
		...
		p.before('<span class="section-title">'+$(this).data('title')+'</span>');
		...
		jQuery.ajax({
		  ...
		  success: function(data ) {
			if(data.msg =='success') {
				...
				setTimeout(function(){ jQuery('.ajax-response').text('').hide(); location.reload(); }, 500);
